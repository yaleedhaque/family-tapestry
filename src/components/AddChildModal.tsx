...
interface AddChildModalProps {
  ...
  parentBName: string;
  persons: PersonLike[];
  onAdd: (person: PersonLike, relationshipType: string) => void;
  onClose: () => void;
}

export default function AddChildModal({ parentAName, parentBName, persons, onAdd, onClose }: AddChildModalProps) {
  const { t } = useLang();
      ...
      >
        <div className="flex items-center justify-center mb-5">
          <div className="text-center">
            <h2 className="font-display text-xl text-[var(--thread-gold)] font-semibold tracking-wide">Add a Child</h2>
            <p className="text-[10px] uppercase tracking-[0.2em] text-[var(--parchment-dim)] mt-0.5">
              {parentAName}{parentBName ? ` & ${parentBName}` : ""}
            </p>
          </div>
        </div>

        {/* Relationship type — always first */}
        <div className="mb-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(["biological", "adopted", "step"] as const).map((rt) => (
              <button
                key={rt}
                onClick={() => setRelationshipType(rt)}
                className={`py-2 px-3 rounded-lg text-xs font-body font-medium transition-all border ${
                  relationshipType === rt
                    ? "bg-[var(--thread-gold)] text-[var(--tapestry-bg)] border-[var(--thread-gold)] shadow-[0_0_12px_rgba(201,165,68,0.3)]"
                    : "bg-white/[0.03] text-[var(--parchment-dim)] border-[var(--thread-gold-dim)]/20 hover:border-[var(--thread-gold-dim)]/50"
                }`}
              >
                {t(`rel.${rt}`)}
              </button>
